...
export default function AddProduct() {
            ...
            useEffect(()=>{
                async function fetchData(){
                    const response = await fetch(
                        `/api/getSingleProduct?slug=${slug}`
                      );
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // if (!formData.mainImage) validationErrors.mainImage = "Main Image is required";
    ...
    if (Object.keys(validationErrors).length === 0) {
      ...
      try {
        const res = await axios.put(`/api/updateProduct?id=${formData.id}`, formData);
  ...
  return (
    <div className="container mx-auto">
      <h1 className="text-black text-[32px] font-bold">Update Product</h1>
